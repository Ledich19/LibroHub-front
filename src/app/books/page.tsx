...
const Books = async () => {
  ...
  const { data } = await getClient().query<BooksWithFiltersQuery>({ query: BOOKS_WITH_FILTER_QUERY });

  // console.log(data.booksFilters);

  return (
    <>
      <Filters filtersData={data?.booksFilters ?? []} />
